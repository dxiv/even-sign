...
- **`package.json`** and **`app.json`** versions should match (currently **1.2.0**). Hub **`evenhub pack`** requires **`app.json`** `version` in **`x.y.z`** form (no prerelease suffix). `app.json` uses `package_id` **`com.dxiv.gloss`**.
